...
customer(nama,alamat,telp,email) 
            VALUES(?,?,?,?)");
    ...
    public function update() {
        $conn = new Connection();
        $mysqli = $conn->getMysqli();
        $stmt = $mysqli->prepare("UPDATE customer SET 
nama=?,alamat=?,telp=?,
            email=? WHERE
           id=?");
    ...
    public static function load($id) {
        $conn = new Connection();
        $mysqli = $conn->getMysqli();
        $stmt = $mysqli->prepare("SELECT id,nama,alamat,telp,email FROM 
customer WHERE id=?");
        $stmt->bind_param("i", $id);
        $stmt->execute();
        $stmt->bind_result($id, $nama, $alamat, $telp, $eamil);
        $customer = null;
        while ($r = $stmt->fetch()) {
